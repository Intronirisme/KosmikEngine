...
class colisionMap:
	...
	def update(self):
		...
		for colision in self.dictDynamic.values():
	...
	def lookForAct(self, posORTHO):
		corner = [(posORTHO[0], posORTHO[1]),(posORTHO[0]+posORTHO[2], posORTHO[1]),(posORTHO[0], posORTHO[1]+posORTHO[3]),(posORTHO[0]+posORTHO[2], posORTHO[1]+posORTHO[3])]
		for coin in corner:
			colorCoin = self.colisionMap.get_at(coin)
			if colorCoin[0] == 0 and colorCoin[2] == 255 or colorCoin[2] == 254 or colorCoin[2] == 253:
				return True
		return False
