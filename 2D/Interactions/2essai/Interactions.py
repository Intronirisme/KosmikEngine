...
class carre:
	def __init__(self, posORTHO, posISO, couleur, vitesse, colision):
		self.posORTHO = posORTHO
		self.posISO = posISO
		self.img = pygame.Surface((self.posISO[2],self.posISO[3]))
		self.img.fill(couleur)
		self.vitesse = vitesse
		self.colision = colision
		self.lastFlag = 'nothing'
		self.lastAlpha = 255
	...
	def haut(self):
		...
		self.lastFlag, self.lastAlpha = self.colision.colisionAction(self.posORTHO, self.posISO)
		if self.lastFlag == 'wall':
			self.posISO.move_ip(0,self.vitesse)
			self.posORTHO.move_ip(self.vitesse, self.vitesse)
...
'''les ressources'''
#la carte
fond = pygame.image.load(path.join('data','map','sallesConjointes.png')).convert() #l'originale
fond.set_alpha(190)
background = fond.copy() #la copie sur laquelle on dessine
frame = pygame.Surface((465,310)).convert()
frame.fill((0,0,0))
allMap = pygame.Surface((465,310)).convert()
#la carte de colision
ImgColMap = pygame.image.load(path.join('data','colision','sallesConjointesColMap.png')).convert()
colMap = colisionMap(ImgColMap)
#les murs
...
perso = carre(pygame.Rect(5,5,12,12), pygame.Rect(75,115,16,50) ,(255,0,0), 2, colMap)
